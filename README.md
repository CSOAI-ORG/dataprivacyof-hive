...
```
L7  PRESENTATION    privacy purple + GDPR blue (Open Design)
L6  ORCHESTRATION   Hermes sub-context (Kimi K2.6 / DeepSeek V3.5 / local)
L5  DOMAIN MCP      dataprivacy-ai-mcp, gdpr-compliance-ai-mcp, hipaa-compliance-mcp
L4  AGENT MEMORY    agentmemory (v0.9.26) — isolated mode
L3  KNOWLEDGE GRAPH Cognee subgraph — scope: data subjects, processing activities, lawful bases, GDPR articles, DSRs
L2  VERSIONED HIST  Memoria (v0.4.0) — namespace "dataprivacyof"
...
```bash
git clone https://github.com/CSOAI-ORG/dataprivacyof-hive
cd dataprivacyof-hive
...
`https://dataprivacyof.ai/.well-known/agent-card.json` — see `agent-card.json`.
...
Bundle with biasdetectionof.ai + accountabilityof.ai as GDPR+EU AI Act package.
